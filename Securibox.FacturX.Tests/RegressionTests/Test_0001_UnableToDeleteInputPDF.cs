...
namespace Securibox.FacturX.Tests.RegressionTests
{
    /*
     * Regression 0001
     * Date: 2026.05.14
     * GitHub issue: https://github.com/Securibox/facturx/issues/61
     *
     * Short description: the FacturxExporter object keeps the source and
     * destination streams alive making it impossible to delete the underlying
     * files even after exporter stream was disposed of.
     *
     * Expected behavior: the FacturxExporter object should release all the
     * associated streams once the exporter stream is disposed.
     */
    internal class Test_0001_UnableToDeleteInputPDF
    {
        ...
        private static readonly string tmpPDFStreamFile = Path.GetTempFileName();
        private static readonly string tmpStreamFile = Path.GetTempFileName();

        [TearDown]
        public void Teardown()
        {
            if (File.Exists(tmpPDFStreamFile))
            {
                File.Delete(tmpPDFStreamFile);
            }
        ...
        public static SpecificationModels.Basic.CrossIndustryInvoice GetModel()
        {
        ...
        [Test]
        [Order(1)]
        public async Task UsingBlocks()
        {
            File.Copy(srcFile, tmpPDFStreamFile, overwrite: true);
            var invoice = GetModel();

            var exporter = new FacturxExporter();
            using (var outStream = new FileStream(tmpStreamFile, FileMode.Create))
            {
                using (
        ...
        [Test]
        [Order(2)]
        public async Task UsingStatements()
        {
            File.Copy(srcFile, tmpPDFStreamFile, overwrite: true);
            var invoice = GetModel();
