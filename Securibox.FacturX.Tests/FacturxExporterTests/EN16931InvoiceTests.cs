...
namespace Securibox.FacturX.Tests.FacturxExporterTests
{
    internal class EN16931InvoiceTests
    {
        ...
        {
            Securibox.FacturX.SpecificationModels.EN16931.CrossIndustryInvoice invoice =
                new Securibox.FacturX.SpecificationModels.EN16931.CrossIndustryInvoice()
                {
        ...
        public async Task WriteData_EN16931_SUCCESS()
        {
            ...
            Securibox.FacturX.SpecificationModels.EN16931.CrossIndustryInvoice invoice =
                GetInvoice_SpecificationModels();
        ...
        public void AssertWrittenData_EN16931_SUCCESS()
        {
            ...
            var en16931Invoice =
                ...
                as Securibox.FacturX.SpecificationModels.EN16931.CrossIndustryInvoice;

            Assert.That(en16931Invoice is not null);

            Assert.That(en16931Invoice!.ExchangedDocument.ID.Value, Is.EqualTo("2023-6013"));
            Assert.That(
                en16931Invoice!.ExchangedDocument.IssueDateTime.DateTimeString.Value,
                Is.EqualTo("20230920")
            );
            Assert.That(
                en16931Invoice!.ExchangedDocument.IssueDateTime.DateTimeString.Format,
                Is.EqualTo("102")
            );
            ...
            Assert.That(
                en16931Invoice!.ExchangedDocument.IncludedNote.ElementAt(0).Content,
                Is.EqualTo("SASU au capital de 200000€")
            );
            Assert.That(
                en16931Invoice!.ExchangedDocument.IncludedNote.ElementAt(0).SubjectCode,
                Is.EqualTo("REG")
            );

            Assert.That(
                en16931Invoice!.ExchangedDocument.IncludedNote.ElementAt(1).Content,
                Is.EqualTo("R.C.S Paris 123 456 789")
            );
            Assert.That(
                en16931Invoice!.ExchangedDocument.IncludedNote.ElementAt(1).SubjectCode,
                Is.EqualTo("ABL")
            );

            Assert.That(
                en16931Invoice!.ExchangedDocument.IncludedNote.ElementAt(2).Content,
                Is.EqualTo("2, rue de la Paix – 75000 Paris – Tel: +33 1 01 12 34 56")
            );
            Assert.That(
                en16931Invoice!.ExchangedDocument.IncludedNote.ElementAt(2).SubjectCode,
                Is.EqualTo("AAI")
            );

            Assert.That(
                en16931Invoice!.ExchangedDocument.IncludedNote.ElementAt(3).Content,
                Is.EqualTo("APE 5510Z – TVA FR40123456824")
            );
            Assert.That(
                en16931Invoice!.ExchangedDocument.IncludedNote.ElementAt(3).SubjectCode,
                Is.EqualTo("AAI")
            );

            Assert.That(
                en16931Invoice!.ExchangedDocument.IncludedNote.ElementAt(4).Content,
                Is.EqualTo(
                    "La loi n°92/1442 du 31 décembre 1992 nous fait l’obligation de vous indiquer que le non-respect des conditions de paiement entraine des intérêts de retard suivant modalités et taux défini par la loi."
                )
            );
            Assert.That(
                en16931Invoice!.ExchangedDocument.IncludedNote.ElementAt(4).SubjectCode,
                Is.EqualTo("PMD")
            );

            Assert.That(
                en16931Invoice!.ExchangedDocument.IncludedNote.ElementAt(5).Content,
                Is.EqualTo(
                    "Une indemnité forfaitaire de 40€ sera due pour frais de recouvrement en cas de retard de paiement."
                )
            );
            Assert.That(
                en16931Invoice!.ExchangedDocument.IncludedNote.ElementAt(5).SubjectCode,
                Is.EqualTo("PMT")
            );
            ...
            Assert.That(
                en16931Invoice!.SupplyChainTradeTransaction.IncludedSupplyChainTradeLineItem.Count(),
                Is.EqualTo(2)
            );

            var line1 =
                en16931Invoice!.SupplyChainTradeTransaction.IncludedSupplyChainTradeLineItem.ElementAt(
                    0
                );
            ...
            Assert.That(
                en16931Invoice!.SupplyChainTradeTransaction.ApplicableHeaderTradeSettlement.ApplicableTradeTax.Count(),
                Is.EqualTo(2)
            );
            ...
            Assert.That(
                en16931Invoice!.SupplyChainTradeTransaction.ApplicableHeaderTradeSettlement.SpecifiedTradeSettlementHeaderMonetarySummation.TaxTotalAmount.Count(),
                Is.EqualTo(1)
            );
            Assert.That(
                en16931Invoice!
