...
namespace Securibox.FacturX.Tests.FacturxExporterTests
{
    internal class BasicInvoiceTests
    {
        ...
        {
            var invoice = new Securibox.FacturX.SpecificationModels.Basic.CrossIndustryInvoice()
            {
        ...
        public void AssertWrittenData_Basic_SUCCESS()
        {
            ...
            var basicInvoice =
                ...
                as Securibox.FacturX.SpecificationModels.Basic.CrossIndustryInvoice;

            Assert.That(basicInvoice is not null);

            Assert.That(basicInvoice!.ExchangedDocument.ID.Value, Is.EqualTo("2023-6013"));
            Assert.That(
                basicInvoice!.ExchangedDocument.IssueDateTime.DateTimeString.Value,
                Is.EqualTo("20230920")
            );
            Assert.That(
                basicInvoice!.ExchangedDocument.IssueDateTime.DateTimeString.Format,
                Is.EqualTo("102")
            );
            ...
            Assert.That(
                basicInvoice!.ExchangedDocument.IncludedNote.ElementAt(0).Content,
                Is.EqualTo("SASU au capital de 200000€")
            );
            Assert.That(
                basicInvoice!.ExchangedDocument.IncludedNote.ElementAt(0).SubjectCode,
                Is.EqualTo("REG")
            );

            Assert.That(
                basicInvoice!.ExchangedDocument.IncludedNote.ElementAt(1).Content,
                Is.EqualTo("R.C.S Paris 123 456 789")
            );
            Assert.That(
                basicInvoice!.ExchangedDocument.IncludedNote.ElementAt(1).SubjectCode,
                Is.EqualTo("ABL")
            );

            Assert.That(
                basicInvoice!.ExchangedDocument.IncludedNote.ElementAt(2).Content,
                Is.EqualTo("2, rue de la Paix – 75000 Paris – Tel: +33 1 01 12 34 56")
            );
            Assert.That(
                basicInvoice!.ExchangedDocument.IncludedNote.ElementAt(2).SubjectCode,
                Is.EqualTo("AAI")
            );

            Assert.That(
                basicInvoice!.ExchangedDocument.IncludedNote.ElementAt(3).Content,
                Is.EqualTo("APE 5510Z – TVA FR40123456824")
            );
            Assert.That(
                basicInvoice!.ExchangedDocument.IncludedNote.ElementAt(3).SubjectCode,
                Is.EqualTo("AAI")
            );

            Assert.That(
                basicInvoice!.ExchangedDocument.IncludedNote.ElementAt(4).Content,
                Is.EqualTo(
                    "La loi n°92/1442 du 31 décembre 1992 nous fait l’obligation de vous indiquer que le non-respect des conditions de paiement entraine des intérêts de retard suivant modalités et taux défini par la loi."
                )
            );
            Assert.That(
                basicInvoice!.ExchangedDocument.IncludedNote.ElementAt(4).SubjectCode,
                Is.EqualTo("PMD")
            );

            Assert.That(
                basicInvoice!.ExchangedDocument.IncludedNote.ElementAt(5).Content,
                Is.EqualTo(
                    "Une indemnité forfaitaire de 40€ sera due pour frais de recouvrement en cas de retard de paiement."
                )
            );
            Assert.That(
                basicInvoice!.ExchangedDocument.IncludedNote.ElementAt(5).SubjectCode,
                Is.EqualTo("PMT")
            );
            ...
            Assert.That(
                basicInvoice!.SupplyChainTradeTransaction.IncludedSupplyChainTradeLineItem.Count(),
                Is.EqualTo(2)
            );

            var line1 =
                basicInvoice!.SupplyChainTradeTransaction.IncludedSupplyChainTradeLineItem.ElementAt(
                    0
                );
            ...
            Assert.That(
                basicInvoice!.SupplyChainTradeTransaction.ApplicableHeaderTradeSettlement.ApplicableTradeTax.Count(),
                Is.EqualTo(2)
            );
            ...
            Assert.That(
                basicInvoice!.SupplyChainTradeTransaction.ApplicableHeaderTradeSettlement.SpecifiedTradeSettlementHeaderMonetarySummation.TaxTotalAmount.Count(),
                Is.EqualTo(1)
            );
            Assert.That(
                basicInvoice!
