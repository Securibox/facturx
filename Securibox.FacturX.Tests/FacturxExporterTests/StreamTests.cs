...
namespace Securibox.FacturX.Tests.FacturxExporterTests
{
    internal class StreamTests
    {
        ...
        private static readonly string srcFile = Path.Combine(path, "2023-6013_facture.pdf");
        ...
        public async Task Exporter_File_Input_Should_Be_Disposed()
        {
            ...
            using var stream = exporter.CreateFacturXStream(tmpFile, invoice, $"SEPEM: Invoice ");

            using var fileStream = new FileStream(dstFile, FileMode.Create);
            await stream.CopyToAsync(fileStream);

            // Assert
            Assert.DoesNotThrow(() => File.Delete(tmpFile));
        }
    }
}
