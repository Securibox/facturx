...
namespace Securibox.FacturX.Core
{
    internal abstract class InvoiceBuilder
    {
        ...
        protected InvoiceBuilder(FacturXConformanceLevelType conformanceLevelType, XmlDocument xmlDocument)
        {
            ...
            ExchangedDocumentNode = xmlDocument.SelectSingleNode("//*[local-name() = 'ExchangedDocument']")!;
            ExchangedDocumentNoteList = ExchangedDocumentNode?.SelectNodes("*[local-name() = 'IncludedNote']");

            ExchangedDocumentContextNode = xmlDocument.SelectSingleNode("//*[local-name() = 'ExchangedDocumentContext']")!;
            ...
            TaxDistributionNodeList = TradeSettlementNode.SelectNodes("*[local-name() = 'ApplicableTradeTax']");

            MonetarySummationNode = TradeSettlementNode.SelectSingleNode("*[local-name() = 'SpecifiedTradeSettlementHeaderMonetarySummation']")!;

            LineNodeList = xmlDocument.SelectNodes("//*[local-name() = 'IncludedSupplyChainTradeLineItem']");
        }
        ...
        protected Models.Minimum.ProccessControl GetProccessControl()
        {
            var businessProccessType = ExchangedDocumentContextNode
              .SelectSingleNode("*[local-name() = 'BusinessProcessSpecifiedDocumentContextParameter']")?
            ...
            if (_conformanceLevelType == FacturXConformanceLevelType.Extended)
            {
                ...
                var testIndicatorNode = ExchangedDocumentContextNode
                 .SelectSingleNode("*[local-name() = 'TestIndicator']");
        ...
        {
            var invoiceNumber = ExchangedDocumentNode.SelectSingleNode("*[local-name() = 'ID']")!.InnerText;
            var invoiceType = ExchangedDocumentNode.SelectSingleNode("*[local-name() = 'TypeCode']")!.InnerText;
            ...
            else if (_conformanceLevelType == FacturXConformanceLevelType.BasicWL
            ...
            else
            {
                ...
                var invoiceName = ExchangedDocumentNode.SelectSingleNode("*[local-name() = 'Name']")?.InnerText;
                header.AddInvoiceName(invoiceName);

                var copyIndicatorNode = ExchangedDocumentNode.SelectSingleNode("*[local-name() = 'CopyIndicator']");
                if (copyIndicatorNode != null)
                {
                    var copyIndicatorString = copyIndicatorNode.SelectSingleNode("*[local-name() = 'Indicator']")!.InnerText;
                    bool.TryParse(copyIndicatorString, out bool copyIndicator);

                    header.AddCopyIndicator(copyIndicator);
                }

                var languageNodeList = ExchangedDocumentNode.SelectNodes("*[local-name() = 'LanguageID']");
                ...
                var effectiveSpecifiedPeriodNode = ExchangedDocumentNode.SelectSingleNode("*[local-name() = 'EffectiveSpecifiedPeriod']");
                if (effectiveSpecifiedPeriodNode != null)
                {
                    ...
                    var effectiveSpecifiedPeriod = XmlParsingHelpers.ExtractDateTime(completeDateTime);
        ...
        internal Models.Minimum.DirectDebit GetDirectDebit()
        {
            ...
            {
                var creditorReferenceId = TradeSettlementNode.SelectSingleNode("*[local-name() = 'CreditorReferenceID']")?.InnerText;
                var paymentReference = TradeSettlementNode.SelectSingleNode("*[local-name() = 'PaymentReference']")?.InnerText;
                var taxCurrencyCode = TradeSettlementNode.SelectSingleNode("*[local-name() = 'TaxCurrencyCode']")?.InnerText;

                if (_conformanceLevelType == FacturXConformanceLevelType.BasicWL
                || _conformanceLevelType == FacturXConformanceLevelType.Basic
                || _conformanceLevelType == FacturXConformanceLevelType.EN16931)
                {
                    return new Models.BasicWL.DirectDebit(currencyCode, creditorReferenceId, paymentReference, taxCurrencyCode);
                }
                else
                {
                    var invoiceIssuerReference = TradeSettlementNode.SelectSingleNode("*[local-name() = 'InvoiceIssuerReference']")?.InnerText;
                    return new Models.Extended.DirectDebit(currencyCode, creditorReferenceId, paymentReference, taxCurrencyCode, invoiceIssuerReference);
                }
            }
        }
        #endregion
        ...
        private IEnumerable<ShippingTransportation>? GetShippingTransportationList()
        {
            ...
            {
                var modeCode = shippingTransportationNode.SelectSingleNode("*[local-name() = 'ModeCode']")?.InnerText;
        ...
        private Models.BasicWL.DeliveryPeriod? GetInvoicingPeriod()
        {
            ...
            if (_conformanceLevelType == FacturXConformanceLevelType.Extended)
            {
                var description = deliveryPeriodNode.SelectSingleNode("*[local-name() = 'Description']")?.InnerText;
        ...
        protected Models.BasicWL.DeliveryDetails? GetDeliveryDetails()
        {
            ...
            var actualDeliveryEventNode = TradeDeliveryNode
               .SelectSingleNode("*[local-name() = 'ActualDeliverySupplyChainEvent']");
            ...
            var deliveryType = TradeAgreementNode
               ...
               .SelectSingleNode("*[local-name() = 'DeliveryTypeCode']")?.InnerText;
            ...
            if (_conformanceLevelType == FacturXConformanceLevelType.BasicWL || _conformanceLevelType == FacturXConformanceLevelType.Basic)
            {
                ...
                {
                    DespatchAdviceReference = ReferenceFactory.ConvertInvoiceReference(References.ReferenceType.DespatchAdviceReference) as Models.BasicWL.DespacthAdviceReference,
                    DeliverToAddress = TradePartyFactory.ConvertActor(TradePartyType.DeliverToAddress) as Models.BasicWL.DeliverToAddress,
            ...
            else if (_conformanceLevelType == FacturXConformanceLevelType.EN16931)
            {
                return new Models.EN16931.DeliveryDetails
                {
                    ...
                    DeliveryDate = actualDeliveryDate,
                    InvoicingPeriod = GetInvoicingPeriod(),
                    ReceivingAdviceReference = ReferenceFactory.ConvertInvoiceReference(References.ReferenceType.ReceivingAdviceReference) as Models.EN16931.ReceivingAdviceReference,
                };
            }
            else if (_conformanceLevelType == FacturXConformanceLevelType.Extended)
            {
                ...
                {
                    DeliverToAddress = TradePartyFactory.ConvertActor(TradePartyType.DeliverToAddress) as Models.Extended.DeliverToAddress,
                    DeliverFromAddress = TradePartyFactory.ConvertActor(TradePartyType.DeliverFromAddress) as Models.Extended.DeliverFromAddress,
                    UltimateDeliverToAddress = TradePartyFactory.ConvertActor(TradePartyType.UltimateDeliverToAddress) as Models.Extended.UltimateDeliverToAddress,
                    DeliveryDate = actualDeliveryDate,
                    InvoicingPeriod = GetInvoicingPeriod() as Models.Extended.DeliveryPeriod,
                    DespatchAdviceReference = ReferenceFactory.ConvertInvoiceReference(References.ReferenceType.DespatchAdviceReference) as Models.Extended.DespacthAdviceReference,
                    ReceivingAdviceReference = ReferenceFactory.ConvertInvoiceReference(References.ReferenceType.ReceivingAdviceReference) as Models.Extended.ReceivingAdviceReference,
                    DeliveryNoteReference = ReferenceFactory.ConvertInvoiceReference(References.ReferenceType.DeliveryNoteReference) as Models.Extended.DeliveryNoteReference,
        ...
        #region PaymentTerms
        private PaymentPenaltyDiscount? GetPaymentPenaltyDiscount(XmlNode paymentPenaltyDiscountNode, bool isDiscount)
        {
            var basisDateTimeNode = paymentPenaltyDiscountNode.SelectSingleNode("*[local-name() = 'BasisDateTime']");
            ...
            var actualDiscountAmountNode = isDiscount ? paymentPenaltyDiscountNode.SelectSingleNode("*[local-name() = 'ActualDiscountAmount']")
              : paymentPenaltyDiscountNode.SelectSingleNode("*[local-name() = 'ActualPenaltyAmount']");
            ...
            var basisPeriodMeasureNode = paymentPenaltyDiscountNode.SelectSingleNode("*[local-name() = 'BasisPeriodMeasure']");
            if (basisPeriodMeasureNode != null)
            {
                paymentPenaltyDiscount.BasePeriodMeasure = new PeriodMeasure
                {
                    ...
                    UnitCode = XmlParsingHelpers.ExtractAttribute(basisPeriodMeasureNode, "unitCode"),
                };
            }
        ...
        {
            var paymentPenalty = TradePaymentTermsNode?.SelectSingleNode("*[local-name() = 'ApplicableTradePaymentPenaltyTerms']");
        ...
        {
            var paymentDiscount = TradePaymentTermsNode?.SelectSingleNode("*[local-name() = 'ApplicableTradePaymentDiscountTerms']");
        ...
        {
            var conditions = TradePaymentTermsNode?.SelectSingleNode("*[local-name() = 'Description']")?.InnerText;
            ...
            .SelectSingleNode("*[local-name() = 'DirectDebitMandateID']")?.InnerText;

            if (_conformanceLevelType == FacturXConformanceLevelType.Extended)
            {
                var partialPaymentAmount = default(decimal?);
                var partialPaymentAmountNode = TradePaymentTermsNode?.SelectSingleNode("*[local-name() = 'PartialPaymentAmount']");
                if (partialPaymentAmountNode != null)
                {
                    partialPaymentAmount = XmlParsingHelpers.ExtractDecimal(partialPaymentAmountNode);
                }
        ...
        {
            var ibanId = creditTransferNode.SelectSingleNode("*[local-name() = 'IBANID']")!.InnerText;
            var proprietaryId = creditTransferNode.SelectSingleNode("*[local-name() = 'ProprietaryID']")?.InnerText;
            ...
            var paymentAccountName = creditTransferNode.SelectSingleNode("*[local-name() = 'AccountName']")?.InnerText;
        ...
        {
            var creditTransferNodeList = TradePaymentMeansNode?.SelectNodes("*[local-name() = 'PayeePartyCreditorFinancialAccount']");
        ...
        protected Models.BasicWL.PaymentInstructions? GetPaymentInstructions()
        {
            ...
            if (_conformanceLevelType == FacturXConformanceLevelType.BasicWL || _conformanceLevelType == FacturXConformanceLevelType.Basic)
            {
                var basicWLPaymentInstructions = new Models.BasicWL.PaymentInstructions(paymentTypeCode, debitedAccountIban);
            ...
            var paymentInstructions = new Models.EN16931.PaymentInstructions(paymentTypeCode, debitedAccountIban);
            paymentInstructions.AddEN16931CreditTransferList(creditTransferList?.Cast<Models.EN16931.CreditTransfer>());
            ...
            var paymentCardNode = TradePaymentMeansNode?.SelectSingleNode("*[local-name() = 'ApplicableTradeSettlementFinancialCard']");
            if (paymentCardNode != null)
            {
                var accountNumber = paymentCardNode.SelectSingleNode("*[local-name() = 'ID']")!.InnerText;
                var cardHolderName = paymentCardNode.SelectSingleNode("*[local-name() = 'CardholderName']")?.InnerText;

                var paymentCardInformation = new Models.EN16931.CardInformation(accountNumber, cardHolderName);
        ...
        private Models.BasicWL.TaxDistribution GetTaxDistribution(XmlNode taxDistributionNode)
        {
            ...
            var vatAmountNode = taxDistributionNode.SelectSingleNode("*[local-name() = 'CalculatedAmount']");
            ...
            var baseAmountNode = taxDistributionNode.SelectSingleNode("*[local-name() = 'BasisAmount']");
            ...
            var vatRateNode = taxDistributionNode.SelectSingleNode("*[local-name() = 'RateApplicablePercent']");
            if (vatRateNode != null)
            {
                categoryRate = XmlParsingHelpers.ExtractDecimal(vatRateNode);
            }

            var categoryCode = taxDistributionNode.SelectSingleNode("*[local-name() = 'CategoryCode']")?.InnerText;
            var categoryType = taxDistributionNode.SelectSingleNode("*[local-name() = 'TypeCode']")?.InnerText;

            var addedTaxPointDateCode = taxDistributionNode.SelectSingleNode("*[local-name() = 'DueDateTypeCode']")?.InnerText;

            var exemptionReason = new Reason
            {
                Text = taxDistributionNode.SelectSingleNode("*[local-name() = 'ExemptionReason']")?.InnerText,
                Code = taxDistributionNode.SelectSingleNode("*[local-name() = 'ExemptionReasonCode']")?.InnerText
            };
            ...
            else
            {
                ...
                var vatTaxPointDateNode = taxDistributionNode.SelectSingleNode("*[local-name() = 'TaxPointDate']");
                ...
                var allowanceChargeBaseAmountNode = taxDistributionNode.SelectSingleNode("*[local-name() = 'AllowanceChargeBasisAmount']");
                if (allowanceChargeBaseAmountNode != null)
                {
                    allowanceChargeBaseAmount = XmlParsingHelpers.ExtractDecimal(allowanceChargeBaseAmountNode);
                }

                var lineTotalBaseAmount = default(decimal?);
                var lineTotalBaseAmountNode = taxDistributionNode.SelectSingleNode("*[local-name() = 'LineTotalBasisAmount']");
        ...
        private TotalAmount? GetTotalAmountWithoutVat()
        {
            ...
            var taxBasisTotalAmountNode = MonetarySummationNode.SelectSingleNode("*[local-name() = 'TaxBasisTotalAmount']");
            ...
            if (_conformanceLevelType == FacturXConformanceLevelType.Extended)
            {
                var currency = XmlParsingHelpers.ExtractAttribute(taxBasisTotalAmountNode, "currencyID");
        ...
        private TotalAmount? GetTotalAmountWithVat()
        {
            ...
            if (_conformanceLevelType == FacturXConformanceLevelType.Extended)
            {
                var currency = XmlParsingHelpers.ExtractAttribute(grandTotalAmountNode, "currencyID");
                return new TotalAmountAndCurrency { Amount = amount, Currency = currency };
            }
            return new TotalAmount { Amount = amount };
        }

        private TotalAmountAndCurrency? GetTotalVatAmountInAccountingCurrency(string? invoiceCurrency)
        {
            var totalVatAmountNodes = MonetarySummationNode?.SelectNodes("*[local-name() = 'TaxTotalAmount']")?.Cast<XmlNode>();
            if (totalVatAmountNodes == null || totalVatAmountNodes.Any(x => XmlParsingHelpers.ExtractAttribute(x, "currencyID") == null))
                return null;

            var totalVatAmountInAccountingCurrency = totalVatAmountNodes.FirstOrDefault(x => !XmlParsingHelpers.ExtractAttribute(x, "currencyID")!.Equals(invoiceCurrency));
            ...
            var currency = XmlParsingHelpers.ExtractAttribute(totalVatAmountInAccountingCurrency, "currencyID");

            return new TotalAmountAndCurrency
            {
                Amount = amount,
                Currency = currency
            };
        ...
        protected Models.Minimum.Totals GetTotals(string invoiceCurrency)
        {
            ...
            else 
            {
                ...
                var totalChargesNode = MonetarySummationNode.SelectSingleNode("*[local-name() = 'ChargeTotalAmount']");
                ...
                var paidAmountNode = MonetarySummationNode.SelectSingleNode("*[local-name() = 'TotalPrepaidAmount']");
                ...
                var roundingAmountNode = MonetarySummationNode.SelectSingleNode("*[local-name() = 'RoundingAmount']");
                ...
                if (_conformanceLevelType == FacturXConformanceLevelType.BasicWL || _conformanceLevelType == FacturXConformanceLevelType.Basic)
                {
                    return new Models.BasicWL.Totals
                    {
                        ...
                        TotalAmountWithoutVat = GetTotalAmountWithoutVat(),
                        AmountToBePaid = amountToBePaid,
                        TotalVatAmountInCurrency = GetTotalVatAmountInAccountingCurrency(invoiceCurrency),
                ...
                else
                {
                    ...
                    {
                        TotalVatAmount = totalVatAmount,
                        TotalAmountWithVat = GetTotalAmountWithVat() as Models.Minimum.TotalAmountAndCurrency,
                        TotalAmountWithoutVat = GetTotalAmountWithoutVat() as Models.Minimum.TotalAmountAndCurrency,
                        AmountToBePaid = amountToBePaid,
                        TotalVatAmountInCurrency = GetTotalVatAmountInAccountingCurrency(invoiceCurrency),
                        NetAmountSum = netAmountSum,
                        AllowancesSum = allowanceSum,
        ...
        protected Models.BasicWL.AllowanceCharge GetAllowanceCharge(XmlNode allowanceChargeNode)
        {
            ...
            var categoryTradeTax = allowanceChargeNode.SelectSingleNode("*[local-name() = 'CategoryTradeTax']");

            var vatRate = default(decimal?);
            var vatRateNode = categoryTradeTax?.SelectSingleNode("*[local-name() = 'RateApplicablePercent']");
            if (vatRateNode != null)
            {
                vatRate = XmlParsingHelpers.ExtractDecimal(vatRateNode);
            }

            var vatType = categoryTradeTax?.SelectSingleNode("*[local-name() = 'TypeCode']")?.InnerText;

            var vatCategoryCode = categoryTradeTax?.SelectSingleNode("*[local-name() = 'CategoryCode']")?.InnerText;

            if (_conformanceLevelType == FacturXConformanceLevelType.Extended)
            {
                var sequenceNumeric = allowanceChargeNode.SelectSingleNode("*[local-name() = 'SequenceNumeric']")?.InnerText;

                var baseQuantity = default(QuantityUnit);

                var baseQuantityNode = allowanceChargeNode.SelectSingleNode("*[local-name() = 'BasisQuantity']");
