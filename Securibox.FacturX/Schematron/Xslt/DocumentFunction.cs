...
namespace Securibox.FacturX.Schematron.Xslt
{
    public class DocumentFunction : IXsltContextFunction
    {
        ...
        {
            return ResolveDocumentFunction(
                xsltContext,
                (string)args[0],
                (string)args[1],
                navigator
        ...
        private static XPathNodeIterator? ResolveDocumentFunction(
        ...
        )
        {
            ...
            if (codedbFile != null)
            {
                ...
                using (var codedbStream = new MemoryStream(byteArray))
                {
                    ...
                    if (
                        dolarVariableMatch != null
                        && dolarVariableMatch.Groups != null
                        && dolarVariableMatch.Groups.Count > 0
                    )
                    {
                        var dolarVariable = xsltContext.ResolveVariable(
                            string.Empty,
                            dolarVariableMatch.Groups[1].Value
                        );
                        ...
                        if (variable != null)
                        {
                            ...
                            if (
                                attributeMatch != null
                                && attributeMatch.Groups != null
                                && attributeMatch.Groups.Count > 0
                            )
                            {
                                var attribute = attributeMatch
                                    .Groups[1]
                                ...
                                var result = (XPathNodeIterator)
                                    codeDBNavigator.Evaluate(compiledExpression);
                                return result;
                            }
                        }
                    }
